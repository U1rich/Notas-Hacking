...
```bash
┌──(ulrich㉿DasMausoleum)-[~/Documentos/Apuntes 8vo Semestre/Reversing/vault-door-1]
└─$ cat VaultDoor1.java                                 
import java.util.*;
...
┌──(ulrich㉿DasMausoleum)-[~/Documentos/Apuntes 8vo Semestre/Reversing/vault-door-1]
└─$ cat flag.txt | sort | aek '{print($3)}' | tr -d "'" | tr -d "\n"
Orden «aek» no encontrada. Quizá quiso decir:
  la orden «ack» del paquete deb «ack»
  la orden «aec» del paquete deb «libaec-tools»
  la orden «ark» del paquete deb «ark»
